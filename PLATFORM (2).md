...
Organizations forward emails to Mengu AI via webhook. Supported integrations include SendGrid Inbound Parse, Mailgun Routes, and any custom SMTP service that can POST JSON. Each email is stored as an `incoming_event` with its full content, metadata, and attachments.
...
| Email Ingestion          | `POST /webhooks/email`                                                |
...
| View & Approve Drafts    | `GET /api/v1/events/:id/drafts`, `PATCH /api/v1/drafts/:id/approve`, `PATCH /api/v1/drafts/:id` |
